          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            ...
            >
              ...
              <span className="text-xs sm:text-base font-black tracking-wider uppercase font-sans">
                <span className={isDark ? 'text-white' : theme === 'olive' ? 'text-[#1e2e1b]' : 'text-slate-900'}>Lets</span>
                <span className={theme === 'olive' ? 'text-[#556b2f]' : 'text-emerald-500'}>Hunt</span>
              </span>
            </div>
            ...
            <div className="relative flex-1 max-w-[140px] sm:max-w-[220px]" ref={searchContainerRef}>
              ...
              >
                <Search className={`w-3 h-3 mr-1.5 flex-shrink-0 ${isDark ? 'text-slate-400' : theme === 'olive' ? 'text-[#6e6a5e]' : 'text-slate-500'}`} />
                <input
                  type="text"
                  value={searchQuery}
                ...
                />
                {isSearching && (
                  <div className={`w-3 h-3 border-2 border-t-transparent rounded-full animate-spin ml-1 flex-shrink-0 ${theme === 'olive' ? 'border-[#556b2f]' : 'border-emerald-500'}`} />
                )}
              </div>
              ...
              {showDropdown && searchResults.length > 0 && (
                ...
                >
                  {searchResults.map((loc, idx) => (
                    ...
                    >
                      <MapPin className={`w-3 h-3 flex-shrink-0 ${theme === 'olive' ? 'text-[#556b2f]' : 'text-emerald-500'}`} />
                      <div className="truncate">
                        <span className="font-semibold">{loc.name}</span>
                        <span className={`ml-1 text-[10px] ${isDark ? 'text-slate-400' : theme === 'olive' ? 'text-[#6e6a5e]' : 'text-slate-500'}`}>
            ...
            <div className={`hidden sm:flex items-center gap-1.5 px-2 py-1 rounded-xl border max-w-[130px] md:max-w-[180px] lg:max-w-[220px] flex-shrink-0 ${
              ...
              <MapPin className={`w-3 h-3 flex-shrink-0 ${theme === 'olive' ? 'text-[#556b2f]' : 'text-emerald-500'}`} />
              <span className={`text-[11px] font-bold truncate ${theme === 'olive' ? 'text-[#2d4a27]' : 'text-emerald-600 dark:text-emerald-400'}`}>
              ...
              <button
                ...
                className={`p-0.5 rounded transition-colors ml-0.5 ${
                  isCurrentDefault
                    ? theme === 'olive' ? 'text-[#556b2f]' : 'text-emerald-500'
                    : isDark
              ...
              <button
                ...
                className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-wider transition-all flex items-center gap-1 ${
                  activeTab === 'dashboard' || activeTab === 'details'
                    ? theme === 'olive' ? 'bg-[#556b2f] text-white shadow-md' : 'bg-emerald-600 text-white shadow-md'
                    : isDark
