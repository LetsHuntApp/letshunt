        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 sm:gap-4 relative z-10">
          ...
          <div className="flex flex-col items-center sm:items-start gap-2 w-full lg:w-auto">
            ...
            <div className="flex flex-col items-center sm:items-start gap-1.5 w-full">
              {/* Top Row Badges */}
              {/* Keep the rating and rut badges together on one row. The
                  compact phone sizing leaves room for both without clipping,
                  while the larger layout keeps the fuller visual treatment. */}
              <div className="flex flex-nowrap items-center justify-start gap-1 sm:gap-1.5 w-[calc(100%+0.5rem)] -mx-1 sm:mx-0 sm:w-full overflow-visible">
                <span
                  className={`shrink-0 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg text-[8px] sm:text-xs font-black uppercase tracking-tight sm:tracking-wider border flex items-center gap-0.5 sm:gap-1 whitespace-nowrap ${
                    isExcellentDay
                ...
                >
                  {isExcellentDay && <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 fill-current text-amber-300" />}
                  <span>{currentRating} Hunt Forecast</span>
                </span>
                <button
                  type="button"
                  onClick={() => setIsRutModalOpen(true)}
                  className={`inline-flex shrink-0 items-center gap-0.5 sm:gap-1.5 px-1.5 sm:px-3 py-0.5 sm:py-1.5 rounded-xl text-[8px] sm:text-[13px] font-black uppercase tracking-tight sm:tracking-wider border-2 whitespace-nowrap cursor-pointer hover:scale-[1.04] active:scale-95 transition-all shadow-lg shadow-black/20 ring-2 ring-white/25 hover:ring-white/50 ${rutInfo.badgeStyle}`}
                ...
                >
                  ...
                  <RutPhaseIcon iconName={rutInfo.iconName} className="w-3 h-3 sm:w-5 sm:h-5 flex-shrink-0" />
                  <span>{rutInfo.name}</span>
                  <Info className="w-3 h-3 sm:w-4 sm:h-4 opacity-80 shrink-0" />
                </button>
              </div>
