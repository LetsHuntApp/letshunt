...
export const TrailCameraGallery: React.FC<TrailCameraGalleryProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Gallery Action Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-2xl bg-slate-900/40 backdrop-blur-md border border-slate-700/50">
          ...
          {isSelectMode && (
            <>
              <button
                ...
                className="px-3 py-1.5 rounded-xl text-xs font-bold bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 transition-colors cursor-pointer"
              >
          ...
          {paginatedPhotos.map((photo) => {
            ...
            return (
              <div
                ...
                className={`group relative rounded-2xl overflow-hidden border transition-all duration-200 cursor-pointer shadow-lg aspect-square flex flex-col justify-between ${
                  isSelected
      ...
      {/* Pagination Controls */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/40 backdrop-blur-md border border-slate-700/50 text-xs font-bold">
          <button
            disabled={currentPage === 1}
            onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
            className="px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 disabled:opacity-40 cursor-pointer flex items-center gap-1"
          >
